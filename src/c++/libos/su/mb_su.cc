...
MbWorker::process_request (unsigned long payload)
{
  ...
  // unsigned int nloops = *reinterpret_cast<unsigned int *>(req_addr) *
  // cycles_per_ns;
  unsigned int nloops
      = *reinterpret_cast<unsigned int *> (req_addr) * cycles_per_ns;
  ...
  *reinterpret_cast<uint32_t *> (req_addr) = 0;
  static_cast<rte_mbuf *> ((void *)payload)->l4_len
      = sizeof (uint32_t) * 3; // request ID + resquest type + response size
  return 0;
}
