...
int
Dispatcher::process_request (unsigned long payload)
{
  return ENOTSUP;
}

int
Dispatcher::dequeue (unsigned long *payload)
{
  return ENOTSUP;
}
...
void
insertionSort (RequestType *rtypes[], int n)
{
  int i, j;
  for (i = 1; i < n; ++i)
    {
  ...
  for (unsigned int j = 0; j < n_rtypes; ++j)
    {
      ...
      if (diff > (rtype->prev_demand * .1)
          or (rtype->ratio > 0 and n_active != prev_active))
        {
...
int
Dispatcher::signal_free_worker (int peer_id, unsigned long notif)
{
  ...
  t->windows_mean_ns = (service_time + (t->windows_mean_ns * t->windows_count))
                       / (t->windows_count + 1);
  t->windows_count++;
...
int
Dispatcher::work (int status, unsigned long payload)
{
  return ENOTSUP;
}

int
Dispatcher::enqueue (unsigned long req, uint64_t cur_tsc)
{
  if (dp == DFCFS)
    {
...
inline int
Dispatcher::push_to_rqueue (unsigned long req, RequestType *&rtype,
                            uint64_t tsc)
{
      ...
      for (uint32_t i = 0; i < n_rtypes; ++i)
        {
          ...
          uint64_t d
              = rtypes[i]
                    ->tsqueue[(rtypes[i]->rqueue_tail + 1) & (RQUEUE_LEN - 1)];
  ...
  if (dp == CFCFS)
    {
      ...
      drain_queue (
          rtypes[type_to_nsorder[static_cast<int> (ReqType::UNKNOWN)]]);
    }
  else if (dp == SJF)
    {
          ...
          for (uint32_t i = 0; i < n_rtypes; ++i)
            {
              ...
              uint64_t qstamp
                  = rtypes[i]
                        ->tsqueue[rtypes[i]->rqueue_tail & (RQUEUE_LEN - 1)];
...
int
Dispatcher::dyn_resa_drain_queue (RequestType *&rtype)
{
  auto &group = groups[rtype->type_group];
  uint64_t cur_tsc = rdtscp (NULL);
  while (rtype->rqueue_head > rtype->rqueue_tail and free_peers > 0)
    {
      PSP_DEBUG ("Dispatching "
                 << rtype->rqueue_head - rtype->rqueue_tail << " "
                 << req_type_str[static_cast<int> (rtype->type)]);
...
int
Dispatcher::drain_queue (RequestType *&rtype)
{
  uint64_t cur_tsc = rdtscp (NULL);
  while (rtype->rqueue_head > rtype->rqueue_tail and free_peers > 0)
    {
