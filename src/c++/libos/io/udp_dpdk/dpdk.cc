...
int
dpdk_net_init (const char *app_cfg_filename)
{
  ...
  PSP_INFO ("DPDK reports that "
            << nb_ports
            << " ports (interfaces) are available for the application.");

  return 0;
}

int
init_dpdk_port (uint16_t port_id, rte_mempool *mbuf_pool, uint16_t n_tx_rings,
                uint16_t n_rx_rings)
{
  ...
  std::cout << "mx rx pkt len= " << port_conf.rxmode.max_rx_pkt_len
            << std::endl;
  ...
  */
  ...
  if (nb_rxd != RX_RING_SIZE)
    {
      printf ("Adjusted number of RX descriptors per queue from %d to %u",
              RX_RING_SIZE, nb_rxd);
    }
  if (nb_txd != TX_RING_SIZE)
    {
      printf ("Adjusted number of TX descriptors per queue from %d to %u",
              TX_RING_SIZE, nb_txd);
    }
  ...
  if (0 != ret)
    {
      fprintf (stderr,
               "WARNING: Failed to get the NUMA socket ID for port %d.\n",
               port_id);
      socket_id = 0;
    }
  ...
  for (uint16_t i = 0; i < rx_rings; ++i)
    {
      PSP_OK (rte_eth_rx_queue_setup (port_id, i, nb_rxd, socket_id, rx_conf,
                                      mbuf_pool));
    }
...
int
rte_eth_dev_flow_ctrl_set (uint16_t port_id,
                           const struct rte_eth_fc_conf &fc_conf)
{
  PSP_TRUE (ERANGE, ::rte_eth_dev_is_valid_port (port_id));

  int ret = ::rte_eth_dev_flow_ctrl_set (
      port_id, const_cast<struct rte_eth_fc_conf *> (&fc_conf));
  if (0 == ret)
    {
      return 0;
    }
...
rte_flow *
generate_ipv4_flow (uint16_t port_id, uint16_t rx_q, uint32_t src_ip,
                    uint32_t src_mask, uint32_t dest_ip, uint32_t dest_mask,
...
int
rte_eth_dev_socket_id (int &sockid_out, uint16_t port_id)
{
  sockid_out = 0;
                        ...
                        struct rte_mempool *mb_pool)
{
  ...
  PSP_DEBUG ("configuring rx queue " << rx_queue_id << " on port " << port_id
                                     << " with " << nb_rx_desc << " rx rings");
  int ret = ::rte_eth_rx_queue_setup (port_id, rx_queue_id, nb_rx_desc,
                        ...
                        const struct rte_eth_txconf &tx_conf)
{
  ...
  PSP_DEBUG ("configuring tx queue " << tx_queue_id << " on port " << port_id
                                     << " with " << nb_tx_desc << " tx rings");
  int ret = ::rte_eth_tx_queue_setup (port_id, tx_queue_id, nb_tx_desc,
                                      socket_id, &tx_conf);
  if (0 == ret)
    {
      return 0;
...
int
rte_eal_init (int &count_out, int argc, char *argv[])
{
  count_out = -1;
...
int
wait_for_link_status_up (uint16_t port_id)
{
  PSP_TRUE (ERANGE, ::rte_eth_dev_is_valid_port (port_id));
...
int
print_link_status (FILE *f, uint16_t port_id, const struct rte_eth_link *link)
{
  ...
  if (ETH_LINK_UP == link->link_status)
    {
      const char *const duplex
          = ETH_LINK_FULL_DUPLEX == link->link_duplex ? "full" : "half";
      fprintf (f,
...
int
rte_eth_dev_configure (uint16_t port_id, uint16_t nb_rx_queue,
...
int
rte_eth_link_get_nowait (uint16_t port_id, struct rte_eth_link &link)
{
  link = {};
  PSP_TRUE (ERANGE, ::rte_eth_dev_is_valid_port (port_id));
...
int
rte_eth_dev_mac_addr_add (uint16_t port_id, struct rte_ether_addr &mac_addr)
{
  PSP_TRUE (ERANGE, ::rte_eth_dev_is_valid_port (port_id));
...
int
print_ether_addr (FILE *f, struct rte_ether_addr &eth_addr)
{
  PSP_NOTNULL (EINVAL, f);
...
int
print_dpdk_device_stats (const uint16_t port_id)
{
  int ret;
  ...
  xstats_names = static_cast<rte_eth_xstat_name *> (
      calloc (len, sizeof (*xstats_names)));
  ...
  if (ret < 0 || ret > len)
    {
      free (xstats);
      free (xstats_names);
      printf ("rte_eth_xstats_get_names(%u) len%i failed: %d", port_id, len,
              ret);
      return 1;
    }
  for (int i = 0; i < len; i++)
